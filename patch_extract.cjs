...
// 1. Add the new button for Whitelist
...
// Inject button after blacklist button
...
const startMarker = '                  <div className="mt-8 bg-slate-900/90 p-6 rounded-3xl border border-slate-800/80 shadow-inner max-w-4xl">';
const endMarker = '                          )}\n                        </tbody>\n                      </table>\n                    </div>\n                  </div>';

const startIndex = content.indexOf(startMarker);
const endIndex = content.indexOf(endMarker) + endMarker.length;
const whitelistUI = content.substring(startIndex, endIndex);
            ...
            {advancedSubTab === "whitelist" && (
              <div className="bg-slate-950 text-slate-100 rounded-[2.5rem] p-6 sm:p-9 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.8)] border border-slate-800/80 relative overflow-hidden animate-fade-in backdrop-blur-2xl">
                ...
                <div className="relative z-10 space-y-6">
                  ${whitelistUI.replace('mt-8', 'mt-0').replace('max-w-4xl', 'w-full flex-1').replace('w-full', 'w-full')}
                </div>
              </div>
            )}
`;

// Wait, I replaced max-w-4xl with w-full flex-1 in the whitelist UI to make sure it stretches nicely!
// Let's inject before heartbeat section
content = content.replace(
